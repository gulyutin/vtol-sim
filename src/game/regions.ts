...
import irkutskOsm from '../regions/irkutsk.osm.bin?url';
...
/**
 * Шотозеро (южная Карелия, Пряжинский район): озеро 15,6 × 7,2 км на 91 м, вытянуто с северо-запада
 * на юго-восток, 43 острова; через него течёт Шуя. Берега низкие — сосновые боры на севере, ельники и
 * смешанный лес на юге, болота. Площадка — аэродром у деревни Салменица на юго-восточном берегу.
 * Готового файла OSM нет: дома и лес собираются в браузере (src/ui/placeOsm.ts).
 */
const SHOTOZERO: LocationSpec = {
  regionName: 'Шотозеро',
  site: { lat: 61.7462, lon: 33.1222 },
  siteName: 'аэродром Салменица',
  region: { south: 61.63, west: 32.76, north: 61.93, east: 33.26 },
  // Конец августа: белые ночи прошли, день ещё длинный; сухо — пожароопасный сезон.
  date: '2026-08-25',
  utcOffsetH: 3,
  localHour: 10,
  windSpeedMs: 4,
  windFromDeg: 240,
  temperatureC: 16,
  // Аэродром за пологим увалом: за 9–10 км над водой НСУ уже не видно. Мачта на холме в километре
  // к юго-западу (~180 м) видит и аэродром, и всё озеро до северного и западного берегов.
  relays: [{ kind: 'ground', lat: 61.736, lon: 33.106, antennaM: 10, name: 'на холме у Салменицы' }],
  search: {
    title: 'Поиск грибника в лесу',
    briefing:
      'Пожилой грибник ушёл утром из Салменицы в лес к юго-западу и не вышел. Вероятный район — ельник с болотцами в 5 км от аэродрома, около 0,9 × 0,9 км. Облететь район с тепловизором и отметить найденного. Медведи, волки и лоси тоже тёплые — отличайте их по размеру, форме и повадке.',
    area: [
      { lat: 61.716, lon: 33.04 },
      { lat: 61.724, lon: 33.04 },
      { lat: 61.724, lon: 33.057 },
      { lat: 61.716, lon: 33.057 },
    ],
    // Карельская тайга: медведь, волк, лось; северного оленя здесь почти нет.
    animals: { bear: 2, wolf: 1, moose: 3, deer: 0 },
  },
  fire: {
    title: 'Лесопожарный патруль',
    briefing:
      'Сухая неделя и гроза ночью — в лесах к югу и востоку от озера дымы. Облететь зону патрулирования 7,7 × 7,4 км. Дым виден за километры: доложить о нём кнопкой «Дым» и щелчком по столбу в 3D-виде. Подойдя, подтвердить очаг тепловизором и отметить огневые точки: тлеющие места внутри гари, очаги переброса за кромкой и одиночные тлеющие деревья — у них дыма почти нет, их видно только в тепловом кадре. Дым тепловизор просвечивает. Нагретые солнцем валуны и избушка с топящейся печью тоже тёплые.',
    area: [
      { lat: 61.69, lon: 33.1 },
      { lat: 61.76, lon: 33.1 },
      { lat: 61.76, lon: 33.24 },
    ...
    route: [
      { lat: 61.735, lon: 33.11, heightAglM: 200 },
      { lat: 61.7, lon: 33.115, heightAglM: 200 },
      { lat: 61.697, lon: 33.17, heightAglM: 200 },
      { lat: 61.7, lon: 33.23, heightAglM: 200 },
      { lat: 61.75, lon: 33.23, heightAglM: 200 },
      { lat: 61.755, lon: 33.17, heightAglM: 200 },
    ],
  },
  survey: {
    title: 'Аэрофотосъёмка деревни Салменица',
    briefing: 'Ортофотоплан деревни Салменица и берега Шотозера, участок 1,6 × 1,1 км, штатной камерой. Нужно GSD не хуже 4 см и не меньше 5 годных кадров на 95 % участка.',
    area: [
      { lat: 61.734, lon: 33.08 },
      { lat: 61.744, lon: 33.08 },
      { lat: 61.744, lon: 33.11 },
      { lat: 61.734, lon: 33.11 },
    ],
  },
  delivery: {
    title: 'Доставка через озеро',
    briefing: 'Отвезти груз в деревню Каменнаволок на западном берегу — 14 км над Шотозером. Площадка — на холме над деревней: оттуда видна мачта, и на земле борт на связи. Сесть, разгрузиться и вернуться. Над водой сесть негде: следите за запасом батареи. Обратно аппарат легче на массу груза.',
    destination: { lat: 61.79, lon: 32.87 },
    destinationName: 'площадка над Каменнаволоком',
    route: [{ lat: 61.77, lon: 33.0, heightAglM: 150 }],
  },
  transfer: {
    title: 'Перелёт А → Б',
    briefing: 'Перелёт с аэродрома Салменица (А) через Шотозеро к деревне Новые Пески на северном берегу (Б), 14 км. Точку Б можно перетащить на карте.',
    destination: { lat: 61.8323, lon: 32.9254 },
    destinationName: 'Новые Пески',
    route: [{ lat: 61.79, lon: 33.02, heightAglM: 150 }],
  },
  route: {
    briefing: 'Свободный маршрут с аэродрома Салменица: поставьте точки на карте и задайте высоту над рельефом для каждой. Берега низкие, над водой и болотами сесть негде. Посадка — на аэродроме.',
    route: [
      { lat: 61.76, lon: 33.06, heightAglM: 150 },
      { lat: 61.79, lon: 33.0, heightAglM: 150 },
      { lat: 61.78, lon: 32.95, heightAglM: 150 },
      { lat: 61.75, lon: 33.02, heightAglM: 150 },
    ],
  },
};

/**
 * Иркутск: город на Ангаре ниже плотины ГЭС, водохранилище, Академгородок, Глазково. Площадка —
 * асфальт парковки к северу от павильона Сибэкспоцентра (Байкальская, 253А), ~475 м. Международный
 * аэропорт — в 3 км к северо-востоку: задания уходят от него на запад и юг. Ретранслятор — на
 * мачте 192 м у Академического моста: с неё видно площадку, Глазково, Академгородок и Кайскую рощу
 * (с земли за Ангарой борт за склонами). Дома, дороги, вода и лес — готовый файл OSM (дома на 12 км).
 */
const IRKUTSK: LocationSpec = {
  regionName: 'Иркутск',
  site: { lat: 52.25657, lon: 104.35291 },
  siteName: 'парковка Сибэкспоцентра',
  region: { south: 52.1, west: 104.1, north: 52.4, east: 104.6 },
  // Середина сентября: сухо, прохладно, ветер вдоль долины Ангары с северо-запада.
  date: '2026-09-15',
  utcOffsetH: 8,
  localHour: 11,
  windSpeedMs: 3,
  windFromDeg: 300,
  temperatureC: 12,
  relays: [{ kind: 'ground', lat: 52.2663, lon: 104.295, antennaM: 120, name: 'на мачте 192 м у Академического моста' }],
  search: {
    title: 'Поиск в Кайской роще',
    briefing:
      'Пожилой человек ушёл гулять в Кайскую рощу и не вернулся к вечеру. Вероятный район — сосновый лес на склоне над Иркутом, около 0,9 × 0,9 км, 8,5 км от площадки через Ангару. Облететь район с тепловизором и отметить найденного. Косули тоже тёплые — отличайте их по размеру, форме и повадке.',
    area: [
      { lat: 52.2545, lon: 104.221 },
      { lat: 52.2625, lon: 104.221 },
      { lat: 52.2625, lon: 104.234 },
      { lat: 52.2545, lon: 104.234 },
    ],
    // Городской лесопарк: из крупных — только косули.
    animals: { deer: 1 },
  },
  survey: {
    title: 'Аэрофотосъёмка: Сибэкспоцентр и Солнечный',
    briefing: 'Ортофотоплан участка 1,6 × 1,1 км от Сибэкспоцентра до берега водохранилища в микрорайоне Солнечный штатной камерой. Нужно GSD не хуже 4 см и не меньше 5 годных кадров на 95 % участка. Аэропорт рядом — выше плана не подниматься.',
    area: [
      { lat: 52.247, lon: 104.345 },
      { lat: 52.257, lon: 104.345 },
      { lat: 52.257, lon: 104.368 },
      { lat: 52.247, lon: 104.368 },
    ],
  },
  delivery: {
    title: 'Доставка в Академгородок',
    briefing: 'Отвезти груз на стадион в Академгородке — 5,5 км на запад, через Лисиху, севернее плотины ГЭС. Сесть, разгрузиться и вернуться. Над плотиной не летать. Обратно аппарат легче на массу груза.',
    destination: { lat: 52.2437, lon: 104.2752 },
    destinationName: 'стадион в Академгородке',
    route: [{ lat: 52.252, lon: 104.315, heightAglM: 150 }],
  },
  transfer: {
    title: 'Перелёт А → Б',
    briefing: 'Перелёт с парковки Сибэкспоцентра (А) через город и Ангару у Академического моста на стадион «Локомотив» в Глазкове (Б), 8 км. Над городом сесть негде — держите запас высоты. Аэропорт в 3 км к северо-востоку: туда не уходить. Точку Б можно перетащить на карте.',
    destination: { lat: 52.2891, lon: 104.2507 },
    destinationName: 'стадион «Локомотив»',
    route: [
      { lat: 52.259, lon: 104.31, heightAglM: 150 },
      { lat: 52.262, lon: 104.285, heightAglM: 150 },
    ],
  },
  route: {
    briefing: 'Свободный маршрут над Иркутском от Сибэкспоцентра: поставьте точки на карте и задайте высоту над рельефом для каждой. Аэропорт в 3 км к северо-востоку — маршрут держите западнее и южнее площадки. Посадка — на парковке.',
    route: [
      { lat: 52.2575, lon: 104.328, heightAglM: 150 },
      { lat: 52.261, lon: 104.285, heightAglM: 150 },
      { lat: 52.28, lon: 104.285, heightAglM: 150 },
      { lat: 52.2758, lon: 104.31, heightAglM: 150 },
    ],
  },
};
...
export const REGION_PRESETS: readonly RegionPreset[] = [
  ...
  { id: 'irkutsk', title: 'Иркутск', hint: 'Город на Ангаре: дома, мосты, плотина ГЭС и водохранилище; взлёт с парковки Сибэкспоцентра, ~475 м; аэропорт в 3 км', location: IRKUTSK, osmUrl: irkutskOsm },
  { id: 'shotozero', title: 'Шотозеро', hint: 'Южная Карелия: озеро с островами, боры, ельники и болота; поиск и лесопожарный патруль; площадка — аэродром Салменица, ~100 м', location: SHOTOZERO },
