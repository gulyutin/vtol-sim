...
export interface AircraftSpec {
  ...
  maxBankDeg: number;
  /**
   * Постоянный перекос в самолётном режиме (реактивный момент маршевого винта, асимметрия планера):
   * крен, ° (+ правым крылом вниз), и нос относительно потока, ° (+ правее). Скольжение даёт силу
   * на борт, крен её уравновешивает — путь от этого не меняется. Нет — полёт симметричный.
   */
  cruiseTrim?: { rollDeg: number; slipDeg: number };
...
/**
 * Откуда отсчитывается высота точек маршрута: над рельефом (автопилот огибает рельеф), над морем
 * или от точки взлёта (высота постоянная, между точками — прямая по высоте).
 */
export type AltitudeRef = 'agl' | 'msl' | 'takeoff';

/** Точка маршрута, которую ставит оператор: положение и высота над рельефом. */
export interface RoutePoint extends GeoPoint {
  heightAglM: number;
  /** Высота над морем, м, — у точек с высотой над морем или от точки взлёта (AltitudeRef). */
  altitudeM?: number;
}
